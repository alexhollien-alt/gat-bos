-- Slice 7A Task 0b-suppl-10 -- morning_briefs.user_id add-column (forward, idempotent)
...
--   SELECT count(*) FROM public.morning_briefs WHERE user_id IS NULL;  -- expect 0
--   SELECT column_default FROM information_schema.columns
--    WHERE table_schema='public' AND table_name='morning_briefs' AND column_name='user_id';
--   -- expect 'auth.uid()'
