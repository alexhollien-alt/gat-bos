-- ============================================================
-- DASHBOARD ARCHITECTURE -- PIECE 2 OF 3
-- Schema infrastructure: deals table, opportunities security,
-- relationship health materialized view, triggers, realtime
-- ============================================================
...
--
-- PREREQ: Piece 1 must have run successfully (contacts.health_score exists).
--
-- WHAT THIS DOES (safe, additive, idempotent):
--   1. Defensively detects and adds contacts.user_id if missing
--   2. Adds user_id + RLS to opportunities (security fix -- they had none)
--   3. Creates new deals table for actual escrow contracts
--   4. Creates agent_relationship_health materialized view
--      with column `computed_health_score` (does NOT collide with the
--      manually set contacts.health_score from Piece 1)
--   5. Refresh triggers on interactions and deals
--   6. Adds dashboard tables to Supabase Realtime publication
--
-- HOW TO RUN:
...
-- ============================================================
-- 0. Defensive contacts.user_id check
-- ============================================================
...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE NOTICE 'Run: UPDATE contacts SET user_id = (SELECT id FROM auth.users LIMIT 1) WHERE user_id IS NULL;';
  ELSE
...
--
-- IMPORTANT: the output column is named `computed_health_score`, NOT
-- `health_score`. This avoids a name collision with the manually set
-- contacts.health_score (renamed from temperature in Piece 1). Piece 3
-- defines a wrapper view that coalesces the manual value with the computed
-- value.
...
SELECT
  ...
  -- Final weighted score (renamed to avoid collision with contacts.health_score)
  GREATEST(0, LEAST(100, ROUND(
    (recency_score        * 0.40) +
    (deal_trend_score     * 0.30) +
    (frequency_score      * 0.20) +
    (responsiveness_score * 0.10)
  )))::int AS computed_health_score,
...
CREATE INDEX idx_arh_user_score ON agent_relationship_health(user_id, computed_health_score DESC);
...
-- ============================================================
--
-- 1. Materialized view populated:
--    SELECT count(*) FROM agent_relationship_health;
--
-- 2. Sample of computed scores:
--    SELECT contact_id, computed_health_score, recency_score, deal_trend_score,
--           frequency_score, responsiveness_score, trend_direction
--    FROM agent_relationship_health
--    ORDER BY computed_health_score DESC
--    LIMIT 10;
--
-- 3. opportunities backfill:
--    SELECT count(*) FROM opportunities WHERE user_id IS NULL;
--    -- Should be 0. If not, find and fix the orphans manually.
--
-- 4. deals table is empty and ready:
--    SELECT count(*) FROM deals;
--    -- Should be 0.
--
-- 5. Realtime publication includes the dashboard tables:
--    SELECT tablename FROM pg_publication_tables WHERE pubname = 'supabase_realtime';
--    -- Should include: interactions, opportunities, deals, tasks
--
-- ============================================================
...
--
-- Note: rollback does NOT remove user_id columns. That has to be done manually.
-- ============================================================
