...
- **Fix needed (Phase 1):** Consolidate into a single source of truth. Two options to evaluate: (a) generate the SQL `CASE WHEN` blocks at migration-write time from the TS constant via a codegen step; (b) move the verb-to-type mapping into a Postgres table (e.g. `node_event_verb_map(verb text PRIMARY KEY, projected_type text NOT NULL)`) read by both the trigger and the rebuild function, with the TS file generated from it. Option (b) is heavier but kills the drift surface entirely. Decide during Phase 1 scope.

### [2026-05-20] Task System Phase 0 -- local dev DB drift from prior verification sessions
- **Broken:** Local Supabase dev DB has accumulated cruft from prior Gate 1 verification runs: (a) duplicate `area` row "Agent Partnerships" with id `ac7bd5c9-9a45-4f7b-9840-75e5713f43d7` alongside the seeded canonical `a0000002-0000-4000-8000-000000000002`; (b) three `cadence` rows for "Julie Jarmiolowski" because contacts are nodes (correctly not deduplicated by name), each new test inserts a new contact + cadence; (c) one ad-hoc area "New Area: Personal Development" id `88c099ed-c419-40a8-b80a-e4e54bf95beb` created during the post-fix curl-7 test before the area-writability guard landed. The route's `resolveParent` for "Agent Partnerships" currently matches the duplicate (`ac7bd5c9-...`) not the seeded UUID.
- **Where:** Local Postgres only. Remote / prod unaffected. Tables: `public.nodes` (type='area' duplicates + ad-hoc, type='contact' Julie rows), `public.cadences` (3 Julie rows joined to those contact nodes).
- **Fix needed:** Defer cleanup to Gate 3 start. Gate 3 seeds the 25-agent contact roster and needs a fresh stack anyway. Approach at Gate 3 entry: `supabase db reset --local` (if the destructive-bash hook allows it with explicit Alex approval), OR targeted soft-delete of the drift rows by id, OR keep the drift + filter on `metadata->>'seeded' = 'true'` in seed scripts. Not a Phase 0 code defect.

### [2026-05-20] Task System Phase 0 -- multiple dev server instances during Gate 1 verification
- **Broken:** Three `next dev` instances were running concurrently during Gate 1 testing: PID 776 on :3000, PID 43741 on :3001, and the freshly-spawned one on :3002. The fresh server bumped to :3002 because the first two were already bound. Per Standing Rule 17, when both 3000 and 3001 respond, ask Alex which is active; this session resolved by inspecting `lsof` and routing the final verification curl to :3002 (the only server running the post-edit code).
- **Where:** Process table. No code defect.
- **Fix needed:** Before any deeper Gate 2+ testing in this branch, kill stale `next dev` instances (`pkill -f "next dev"` then restart one fresh). Coordinate dev server lifecycle so only one runs at a time per Rule 17 spirit. Not a Phase 0 code defect.
...
- Closing commit: `b5c36c8` (PR #53, squash-merged 2026-05-21 via `CLAUDE_AUTOMATION_PR_MERGE=53` bypass per AI_AGENT_OPERATING_MODEL.md Section 13).

---

Remaining placeholders in this file: none.
