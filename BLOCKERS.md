...
### [2026-05-18] Schema drift: baseline migration registered but `agent_relationship_health` MV missing on remote
- **Broken:** Baseline migration `20260407012800_baseline.sql` defines `public.agent_relationship_health` (MATERIALIZED VIEW, lines 366-440) and is marked applied in the remote migration registry. But `supabase db push` of a follow-up migration referencing the relation fails with `relation "public.agent_relationship_health" does not exist (SQLSTATE 42P01)`. No subsequent migration drops it.
- **Where:** Remote gat-bos Supabase project. Migration registry vs. actual schema disagree.
- **Discovered:** 2026-05-18 while attempting to migrate the paste-only `dashboard-piece5-mv-lockdown.sql` into a CLI-managed migration per Rule 23. Source file archived to `supabase/_archive/dashboard-piece5-mv-lockdown.sql`.
- **Fix needed:** Investigate why the MV is missing despite the baseline being applied. Options: (a) MV was manually dropped post-baseline outside the migration system, (b) baseline applied partially or errored silently on this DDL block, (c) different remote was the apply target. Run `psql` over the linked URL to enumerate views/materialized views, compare against baseline, decide whether to recreate via a new migration or recover via point-in-time. Once recreated, re-apply the lockdown migration (content preserved in the archived paste-file).
...
### [2026-04-21] Fiona Bigbee + Denise van den Bossche phone/website missing
- **Broken:** Contact block on `/agents/fiona-bigbee` and `/agents/denise-van-den-bossche` renders email only. Page reads via `get_public_agent_by_slug` RPC; `contacts.phone` + `contacts.website_url` are NULL for both. JSON-LD `telephone` + `url` ship as `null`.
- **Where:** `src/app/agents/[slug]/page.tsx` reads `contacts` table via RPC. Wiki canonical: `~/Documents/Alex Hub(Obs)/wiki/agents/fiona-bigbee.md` + `denise-van-den-bossche.md`. (Note: the legacy "AGENTS const" reference in prior versions of this entry is obsolete; data flows from `contacts` table, not a hardcoded object.)
- **Status 2026-05-18:** Wiki pages updated with explicit `Phone` / `Email` / `Website` [PLACEHOLDER:] fields on both agents (Fiona's were missing entirely; Denise's already flagged). Awaiting Alex to provide phone + website values per agent, then UPDATE `contacts` row via `supabase migration new` per Rule 23.
- **Fix needed:** Alex confirms phone + website (or website=none) for each. Update wiki files with values, then write migration `UPDATE contacts SET phone='...', website_url='...' WHERE slug IN ('fiona-bigbee','denise-van-den-bossche');` and `supabase db push`.
