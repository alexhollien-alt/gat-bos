...
### [2026-05-03] Resend webhook lookup misses send_mode='both' rows (Slice 8 Phase 5 Gate 17)
- **Broken:** Phase 5 dry-run send to alex@alexhollienco.com (messages_log `bbd25ec5-e656-46fe-8262-25820940c13e`, status='sent', sent_at 2026-05-03T04:56:18Z) produced zero `message_events` rows and zero `activity_events` ties after >60s. Resend accepted the send and returned fallback ID `08309aa6-55d2-4986-ac7a-5dc23534fd54`, but the webhook handler can never join it back. Root cause: `send_mode='both'` writes the **Gmail** message ID to `messages_log.provider_message_id` while the Resend ID is buried inside `event_sequence[].sent.payload.fallback_message_id` (jsonb). The webhook handler's lookup queries `provider_message_id` only, so the Resend ID never matches and the event is dropped silently.
- **Where:** `src/app/api/webhooks/resend/route.ts:147-152` (lookup queries `messages_log.provider_message_id` only). `src/lib/messaging/send.ts` (the `both` branch -- writes Gmail ID to `provider_message_id`, stashes Resend ID in `event_sequence[].sent.payload.fallback_message_id`). Verification: `scripts/phase-021-gate-17-webhook-check.mjs`.
- **Fix needed (pick one):** (a) Webhook handler falls back to a JSONB query against `event_sequence` when the `provider_message_id` lookup misses: `messages_log` rows where `event_sequence @> '[{"event":"sent","payload":{"fallback_message_id":"<id>"}}]'`. Lowest blast radius. (b) Add a dedicated `resend_message_id text` column to `messages_log`, backfill from `event_sequence`, and have the `both` branch write to both columns. Cleanest long-term but a schema migration. (c) Switch `send_mode='both'` to write the Resend ID to `provider_message_id` and the Gmail ID to `event_sequence`. Inverts the current semantics and breaks any other code that assumes `provider_message_id` = primary provider. Recommendation: **(a)** for Slice 8 Phase 5.6; revisit (b) only if multiple consumers grow this need.
- **Workaround for Phase 5 dry-run:** None needed -- the actual email lands in inbox (Gate 18). Webhook telemetry gap is internal-observability only and does not block ship.
