...
- **Global scope**: `/Users/alex/crm/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/Users/alex/crm/.claude/notes/`)
...
2. Glob `/Users/alex/crm/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/Users/alex/crm/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/Users/alex/crm/.claude/notes/` — works in any directory
