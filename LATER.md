...
- **Why deferred:** Pure spec/harness mismatch; no functional bug. Real prod path (Supabase Auth verify of Resend-delivered email) does not depend on `event_sequence.body_html`. Closing 7C with WARN logged here per Standing Rule 5 ("ship" + "consequence-based gate" -- WARN does not block prod write).

### [2026-05-02] (Slice 7C plumbing) Per-user OAuth token threading in src/lib/gmail/oauth.ts
- **Where:** `src/lib/gmail/oauth.ts:24` (`USER_ID = "b735d691-4d86-4e31-9fd3-c2257822dca3"` constant). Four downstream callsites read it implicitly: `src/app/api/auth/gmail/callback/route.ts:37`, `src/lib/calendar/client.ts:41,57,116,138`, `src/lib/messaging/adapters/gmail.ts:18,29,78`, `src/lib/gmail/sync-client.ts:32,43,85,101,144,169`.
- **What:** Add `userId: string` parameter to `saveTokens` / `loadTokens` / `touchLastUsed` signatures. Resolve at call time via `tenantFromRequest` (user-path) or `accounts.owner_user_id` lookup (cron path, mirroring the `resolveAccountOwnerId()` pattern in `src/app/api/cron/touchpoint-reminder/route.ts`). Drop the module-level constant.
- **Why deferred:** Single Google account today (Alex's Gmail/Calendar). The Slice 7A migration `20260427300212_slice7a_oauth_tokens_0_user_id_type_fix.sql` flipped `oauth_tokens.user_id` text->uuid and remapped the literal `'alex'` to Alex's owner UUID; the application code now matches. Threading through the API surface is needed only when a 2nd Google account lands (additional brokerage admin, etc.).
...
### [2026-04-30] (Slice 7A closure) Migration registry drift -- 17 remote-only timestamps + add_missing_timestamps push -- DONE 2026-05-03 (yellow-flag plumbing PR)
- **Closing context:** Pre-flight `supabase migration list --linked` on 2026-05-03 returned 106 local files == 106 paired remote rows, drift=0. Migration `20260429194551_add_missing_timestamps.sql` is git-tracked AND already remote-applied (Local==Remote in the registry). The 17 remote-only timestamps from the original entry were resolved by Slice 7A.5's repair pass on 2026-05-01. No `supabase db push` needed.
- **Closing PR:** plumbing/yellow-flag-cleanup-2026-05-03 (audit Action 2 subset). LATER.md and BLOCKERS.md hygiene only; no code changes.

### [2026-05-03] (Audit GAP) gh CLI revisit decision -- NO LONGER APPLICABLE 2026-05-03
- **Why no longer applicable:** PR #25 + #26 invocations via `gh pr create` / `gh pr merge --squash --delete-branch` paired with the `CLAUDE_AUTOMATION_PR_MERGE=<n>` bypass worked cleanly end-to-end across two consecutive Slice 8 plumbing PRs on 2026-05-03. The bypass log at `~/.claude/audit/destructive-bash-bypasses.log` carries 7 structured pipe-separated entries through this session. `~/.claude/rules/tool-routing.md` already mandates `gh` for all GitHub operations and the violations catalog already bans `curl https://api.github.com/...` for any operation `gh` covers. No separate revisit needed; the toolchain is settled and the audit GAP is closed by observation.
